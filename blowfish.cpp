...
uint32_t* BlowFish::GetFirstConsts(){
    static uint32_t* FIXED_P = new uint32_t[]{
...
uint32_t** BlowFish::GetSecondConsts(){

    static uint32_t* first_array = new uint32_t[] {
    ...
    static uint32_t* second = new uint32_t[] {
    ...
    static uint32_t* third = new uint32_t[] {
    ...
    static uint32_t* fourth = new uint32_t[] {
    ...
    static uint32_t** S = new uint32_t*[4] {first_array, second, third, fourth};
...
QString BlowFish::Encrypt(QString text) {
    static uint32_t* p = GetFirstConsts();
    static uint32_t** s = GetSecondConsts();

    QByteArray data = text.toUtf8();
    padData(data);
    QByteArray encryptedData;
    for (int i = 0; i < data.size(); i += 8) {
        quint32 xl = toUint32(data.mid(i, 4));
        quint32 xr = toUint32(data.mid(i + 4, 4));
        encryptBlock(xl, xr,p,s);
        encryptedData.append(toByteArray(xl));
        encryptedData.append(toByteArray(xr));
    }
    return encryptedData.toBase64();
}

QString BlowFish::Decrypt(QString ciphertext) {
    static uint32_t* p = GetFirstConsts();
    static uint32_t** s = GetSecondConsts();
    ...
    for (int i = 0; i < data.size(); i += 8) {
        ...
        decryptBlock(xl, xr, p ,s);
        decryptedData.append(toByteArray(xl));
        decryptedData.append(toByteArray(xr));


    }

    QString result = QString::fromUtf8(decryptedData).trimmed();
...
void BlowFish::padData(QByteArray &data) {
    int padLength = 8 - (data.size() % 8);
...
quint32 BlowFish::toUint32(const QByteArray &data) {
    quint32 value = 0;
...
QByteArray BlowFish::toByteArray(quint32 value) {
    QByteArray data;
    for (int i = 3; i >= 0; --i) {
...
void BlowFish::encryptBlock(quint32 &xl, quint32 &xr,uint32_t* &P, uint32_t** &S) {
    for (int i = 0; i < 16; ++i) {
        xl ^= P[i];
        xr ^= F(xl, S);
        qSwap(xl, xr);
...
void BlowFish::decryptBlock(quint32 &xl, quint32 &xr, uint32_t* & P, uint32_t** &S) {
    for (int i = 17; i > 1; --i) {
        xl ^= P[i];
        xr ^= F(xl,S);
        qSwap(xl, xr);
...
quint32 BlowFish::F(quint32 x, uint32_t** & S) {
